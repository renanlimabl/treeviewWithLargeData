    ...
    return (function () {
      ...
      function render(self) {
        ...
        click = function (e) { 
          ...
          // GET ALL ELEMENTS INPUTS OF CHECKBOX
          const allInputs = document.querySelectorAll('input')

          // REMOVE ATTR CHECKED OF ALL INPUTS CHECKBOX
          Array.from(allInputs).forEach(item => item.removeAttribute('checked'))

          
          // GET THE CURRENT TARGET INPUT CHECKBOX AND SET WITH TRUE
          parent.querySelector('input').setAttribute('checked', true)

          // VERIFY IN ALL INPUTS CHECKBOX IF ITS HAVE ATTR CHECKED, SO IF HAVES, CHECKBOX = TRUE OR CHECKBOX = FALSE
          Array.from(allInputs).forEach(item => item.hasAttribute('checked') ? item.checked = true : item.checked = false)

    
          // var checkboxes = Array.from(document.querySelectorAll('input[type="checkbox"]:checked'))
          // checkboxes.forEach(item => item.removeAttribute('checked'))
          // checkboxes.pop().toggleAttribute('checked')
          
          /**
           * get id and text for item select
           */
        ...
        forEach(container.querySelectorAll('.tree-checkbox'), function (node) {
          node.onclick = click;
        });
      }
